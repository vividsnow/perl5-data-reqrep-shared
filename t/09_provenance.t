...
for my $off ('0', '0.0', 'false', 'FALSE', 'off', 'no', '') {
...
# Creating with the fd table nearly full must not store an unreadable provenance.
SKIP: {
...
P
    ...
    for my $v (['Str', 'Data::ReqRep::Shared', 'Data::ReqRep::Shared::Client', 4, 2, 64],
        ...
        like $out, qr/^(?:created\nattached|refused: .*Too many open files)/, '  it either works or says why' or diag $out;
    }
}
...
C
    ...
    require Cwd;
    my $inc = Cwd::abs_path('.');
    system($cc, "-I$inc", "$emdir/prov_zero.c", "-o", "$emdir/prov_zero") == 0
