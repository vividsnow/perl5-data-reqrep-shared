...
#define MAKE_OBJ(class, ptr) \
    ...
    sv_bless(ref, reqrep_stash(aTHX_ class)); \
    RETVAL = ref
...
/* Every syscall would cut a path short at an embedded NUL. */
...
/* A handler died while a reply was owed: the caller never saw the id, so cancel here. */
struct reqrep_inflight { ReqRepHandle *h; SV *obj; uint64_t id; };

static void reqrep_cancel_inflight(pTHX_ void *p) {
    struct reqrep_inflight *f = (struct reqrep_inflight *)p;
    if (f->id && reqrep_handle(aTHX_ f->obj, NULL) == f->h) {
...
new(class, path, req_cap, resp_slots, resp_size, ...)
  ...
  CODE:
    ...
    UV mode = (items > 6 && (SvGETMAGIC(ST(6)), SvOK(ST(6)))) ? SvUV(ST(6)) : 0600;
    if ((mode & ~(UV)07777) || (mode & 0600) != 0600)
        croak("Data::ReqRep::Shared->new: mode %#" UVxf " is not a permission mode the owner can read and write", mode);
...
new_memfd(class, name, req_cap, resp_slots, resp_size, ...)
  ...
  CODE:
    arena_cap = (items > 5 && (SvGETMAGIC(ST(5)), SvOK(ST(5)))) ? (uint64_t)SvUV(ST(5)) : 0;
    if (req_cap > 0xFFFFFFFFU || resp_slots > 0xFFFFFFFFU || resp_size > 0xFFFFFFFFU) croak("Data::ReqRep::Shared->new_memfd: a capacity/size argument exceeds 2^32");
...
recv_multi(self, count)
  ...
  PPCODE:
    ...
    if (count > reqrep_queued_hint(h)) count = reqrep_queued_hint(h);
    if (count > 0) {
...
recv_wait_multi(self, count, ...)
  ...
  PPCODE:
    ...
    if (count > 1 + reqrep_queued_hint(h)) count = 1 + reqrep_queued_hint(h);
    if (count > 1) {
...
unlink(self_or_class, ...)
  ...
  CODE:
    ...
    if (sv_isobject(self_or_class) && sv_derived_from(self_or_class, "Data::ReqRep::Shared")) {
        ReqRepHandle *h = reqrep_handle(aTHX_ SvRV(self_or_class), NULL);
        ...
        path = reqrep_path_arg(aTHX_ ST(1), "Data::ReqRep::Shared->unlink");
    }
...
new(class, path)
  ...
  PREINIT:
    char errbuf[REQREP_ERR_BUFLEN];
  CODE:
    const char *p = reqrep_path_arg(aTHX_ path, "Data::ReqRep::Shared::Client->new");
...
req(self, value)
  ...
  CODE:
    ...
    double deadline = timeout > 0 ? reqrep_monotime() + timeout : 0;
    uint64_t inflight = 0;
    struct reqrep_inflight owed = { h, h_obj, 0 };
    int r;
    ENTER;
...
new(class, path, req_cap, resp_slots, ...)
  ...
  CODE:
    ...
    UV mode = (items > 4 && (SvGETMAGIC(ST(4)), SvOK(ST(4)))) ? SvUV(ST(4)) : 0600;
    if ((mode & ~(UV)07777) || (mode & 0600) != 0600)
        croak("Data::ReqRep::Shared::Int->new: mode %#" UVxf " is not a permission mode the owner can read and write", mode);
...
unlink(self_or_class, ...)
  ...
  CODE:
    ...
    if (sv_isobject(self_or_class) && sv_derived_from(self_or_class, "Data::ReqRep::Shared::Int")) {
        ...
        path = reqrep_path_arg(aTHX_ ST(1), "Data::ReqRep::Shared::Int->unlink");
    }
    if (!path) croak("cannot unlink anonymous or memfd channel");
...
new(class, path)
  ...
  PREINIT:
    char errbuf[REQREP_ERR_BUFLEN];
  CODE:
    const char *p = reqrep_path_arg(aTHX_ path, "Data::ReqRep::Shared::Int::Client->new");
