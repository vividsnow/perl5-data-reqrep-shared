...
# other processes change the slot under them; nothing may leak, hang, or reach
# the wrong request.
...
int main(int argc, char **argv) {
    ...
    ReqRepHandle *srv = is_int ? reqrep_create_int(path, two ? 2 : 16, two ? 2 : 1, 0600, err)
                               : reqrep_create(path, two ? 2 : 16, two ? 2 : 1, 64, 0, 0600, err);
    ...
    if (!strcmp(mode, "stale_acquire")) {
        ...
        pid_t p = fork();
        if (!p) { reqrep_mutex_lock(at()); raise(SIGSTOP); _exit(0); }
        wait_stop(p, "holder");
        pid_t c = fork();
        if (!c) {
            ReqRepHandle *h = at();
            ...
            double t0 = now_ms();
            int sw = reqrep_send_wait(h, "x", 1, false, &cid, 0.3);
            int rw = reqrep_recv_wait(h, &s, &l, &u, &cid, 0.3);
            int on_time = now_ms() - t0 < 1500;
            t0 = now_ms();
        ...
        for (int i = 0; i < 2 * REQREP_STALE_WAKES; i++) { reqrep_mutex_unlock(srv); reqrep_mutex_lock(srv); }
...
my @rows = (
    ...
    [send_clear           => qr/^same_id=0 X=-4 N=1:6 replies=1,-2 busy=0$/, 'a send resuming after clear() cannot reuse the next sender\'s id'],
