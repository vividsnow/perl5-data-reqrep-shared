...
# such replies. Two threads of the receiving process are released on one slot
# together, one holding the stale id, one the live id; the live reply must
# always land and the stale one must never be accepted.
...
static void *replier(void *arg) {
    ...
    long seen = 0;
    for (;;) {
        while (sh->recvd == seen && !sh->stop) reqrep_spin_pause();
